...
for tile in tiles:
    ...
    grid_tiles = glob.glob(os.path.join('/my_volume/FORCE_subset_BW/', '*' + tile + '*.gpkg'))
...
# ANNEX I - how to create tile ID list
...
s2dir = glob.glob(('/force/FORCE/C1/L2/ard/' + tile + '/'), recursive=True)  # Initialize path to Sentinel-2 time series
raster_paths = []  # create object to be filled with rasters to be stacked
tifs = glob.glob(os.path.join(s2dir[0],
                              "*SEN*BOA.tif"))  # i want to ignore the landsat files and stack only sentinel 2 bottom of atmosphere observations
raster_paths.extend(tifs)  # write the ones i need into my object to stack from
years = [int(re.search(r"\d{8}", raster_path).group(0)) for raster_path in
         raster_paths]  # i need to extract the date from each file path...
raster_paths = [raster_path for raster_path, year in zip(raster_paths, years) if
                year <= 20230302]  # ... to cut off at last image 20230302 as i trained my model with this sequence length, this might change in the future with transformers
datacube = [rxr.open_rasterio(raster_path) for raster_path in
            raster_paths]  # i user rxr because the clip function from rasterio sucks
# the datacube is too large for the RAM of my contingent so i need to subset using the 5x5km tiles
grid_tiles = glob.glob(os.path.join('/my_volume/FORCE_subset_BW/', '*' + tile + '*.gpkg'))
for minitile in grid_tiles:  # now i iterate over the small grids
    crop_shape = gpd.read_file(minitile)  # open the corresponding geometry
